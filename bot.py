...
WATCHLIST = [
    ...
    "AVAX-USD", "DOT-USD", "LINK-USD", "MATIC-USD", "DOGE-USD"
]
...
def scan_market():
    ...
    print(f"🔍 Running Institutional Momentum Scanner for {today_str}...\n")

    btc_data = yf.download("BTC-USD", period="1y", progress=False)
    # Ensure scalar values are used for comparison
    btc_ema200 = float(btc_data['Close'].ewm(span=200, adjust=False).mean().iloc[-1])
    btc_current = float(btc_data['Close'].iloc[-1])
    macro_bullish = btc_current > btc_ema200

    if not macro_bullish:
        print("⚠️ MACRO WARNING: Bitcoin is below its 200-day EMA. Market is bearish.")
        msg = f"🚨 *MARKET REGIME ALERT* 🚨\n"
        msg += f"📅 Date: {today_str}\n"
        msg += f"📉 *BTC Status:* Bearish (Below 200 EMA)\n"
    ...
    for ticker in WATCHLIST:
        ...
        try:
            ...
            if (prev_ema50 > prev_ema200) and (40 <= prev_rsi <= 55) and (prev_close > current_open) and (prev_volume > (1.2 * prev_vol_sma)):
                signal = "🟢 LONG"
                entry = current_price
                stop_loss = entry - (2 * current_atr)
                take_profit = entry + (4 * current_atr)
    ...
    msg = f"🚨 *INSTITUTIONAL CRYPTO SCANNER* 🚨\n"
    msg += f"📅 Date: {today_str}\n"
    msg += f"📊 *BTC Macro:* 🟢 Bullish (Above 200 EMA)\n"
    ...
    if not valid_signals:
        ...
        for i, sig in enumerate(valid_signals, 1):
            ...
            msg += f"📦 *Position Size:* `{sig['size']:.4f}` units\n"
            msg += f"⚠️ *Capital at Risk:* `${sig['risk_amt']:,.2f}`\n"
            msg += f"📝 *Logic:* {sig['reason']}\n"
            msg += "──────────────────\n"
        msg += "💡 *Execution:* Use Demo Trading first. Set TP/SL and walk away."
